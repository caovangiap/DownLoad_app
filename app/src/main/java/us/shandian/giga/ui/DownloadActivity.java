...
import com.example.download_app.databinding.ActivityDownloaderBinding;
import com.nononsenseapps.filepicker.Utils;
...
//            AdsManager.getInstance().setDoNotShowOpen();
    ...
    private void setDefaultDownloadFolder() {
        ...
        spEditor.putString(key_video, new File(getDir(Environment.DIRECTORY_DOWNLOADS), String.valueOf(R.string.download_path_audio_key)).toURI().toString());
        spEditor.putString(key_audio, new File(getDir(Environment.DIRECTORY_DOWNLOADS), SyncStateContract.Constants.ACCOUNT_NAME).toURI().toString());
        Log.d("adsdk",  new File(getDir(Environment.DIRECTORY_DOWNLOADS), SyncStateContract.Constants.ACCOUNT_NAME).toURI().toString());
        spEditor.apply();
