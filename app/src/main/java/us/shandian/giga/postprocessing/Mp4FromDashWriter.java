...
public class Mp4FromDashWriter {
    ...
    public Mp4FromDashWriter(final SharpStream... sources) throws IOException {
        ...
        readersChunks = new Mp4DashReader.Mp4DashChunk[readers.length];
    ...
    public Mp4DashReader.Mp4Track[] getTracksFromSource(final int sourceIndex) throws IllegalStateException {
        if (!parsed) {
    ...
    public void selectTracks(final int... trackIndex) throws IOException {
        ...
        try {
            tracks = new Mp4DashReader.Mp4Track[readers.length];
            for (int i = 0; i < readers.length; i++) {
    ...
    public void build(final SharpStream output) throws IOException {
        ...
        if (tracks.length == 1 && tracks[0].kind == Mp4DashReader.TrackKind.Audio) {
        ...
        for (int i = 0; i < readers.length; i++) {
            ...
            Mp4DashReader.Mp4DashChunk chunk;
        ...
        while (written > 0) {
            ...
            for (int i = 0; i < readers.length; i++) {
                ...
                int j = 0;
                for (; j < limit; j++) {
                    final Mp4DashReader.Mp4DashSample sample = getNextSample(i);
    ...
    private Mp4DashReader.Mp4DashSample getNextSample(final int track) throws IOException {
        ...
        final Mp4DashReader.Mp4DashSample sample = readersChunks[track].getNextSample();
        if (sample == null) {
    ...
    private void makeMdia(final Mp4DashReader.Mdia mdia, final TablesInfo tablesInfo, final boolean is64,
    ...
    private byte[] makeHdlr(final Mp4DashReader.Hdlr hdlr) {
